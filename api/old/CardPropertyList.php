...
define( INPUT_FILE_PATH, './CardPropertyList.json' );
...
try {
    ...
    $CardPropertyList = json_decode( $json_string );

    if ( empty( $CardPropertyList ) ) {
        header("HTTP/1.1 404 Not Found");
        exit(0);
    }

    $result = array( 'data' => $CardPropertyList );  // 返却値の作成
